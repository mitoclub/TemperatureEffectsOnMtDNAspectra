---
title: "Analysis of the whole genomes data in Fishes from Mikhailova et al. 2022"
...
docname <- "II_Whole_Genomes_Data_Analysis"
now <- Sys.time()
...
# Reading codon usage database (L-strand), calculating neutral nucleotide content and reading ecology

```{r codon usage, message=FALSE}
mtDNAcodonUsage <- read.table(here(data_dir, "AllGenesCodonUsageNoOverlap.txt"), header = TRUE, sep = '\t')
mtDNAcodonUsage = mtDNAcodonUsage[mtDNAcodonUsage$Gene != 'ND6' & mtDNAcodonUsage$Gene == 'CytB',] # check Cyt B
...
MaturationMedian <- median(NucAndEco$tm, na.rm = TRUE)
TemperatureMedian <- median(NucAndEco$Temp_avg, na.rm = TRUE)
NucAndEco$TemperatureGroup <- ifelse(NucAndEco$Temp_avg >= TemperatureMedian, "Warm", "Cold")
NucAndEco$LongevityGroup <- ifelse(NucAndEco$tm >= MaturationMedian, "LongLived", "ShortLived")
NucAndEco <- NucAndEco[!is.na(NucAndEco$TemperatureGroup) & !is.na(NucAndEco$LongevityGroup),]

sf6 <- ggplot(data = NucAndEco, aes(x = TemperatureGroup, y = AGskew))+
  geom_boxplot()+
  facet_wrap(~LongevityGroup)+
  theme_test()+
  labs(y = 'AGskew')
sf6
```
