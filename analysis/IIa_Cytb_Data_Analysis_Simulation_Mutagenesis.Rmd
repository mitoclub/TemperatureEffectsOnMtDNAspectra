...
#Rerunning simulation on small subset 

```{r}
WarmSubset <- MutNucAndEcoCb[!is.na(MutNucAndEcoCb$FrA.H) & !is.na(MutNucAndEcoCb$Climate_zone) & !is.na(MutNucAndEcoCb$A_G.H) & MutNucAndEcoCb$Climate_zone %in% c("tropical", "subtropical"),]
WarmFishes <- WarmSubset %>% dplyr::select(c("T_G.H", "T_C.H", "T_A.H", "G_T.H", "G_C.H", "G_A.H", "C_T.H" , "C_G.H", "C_A.H", "A_T.H", "A_G.H", "A_C.H"))
WarmFishes <- WarmFishes %>% summarise_all(mean)


ColdSubset <- MutNucAndEcoCb[!is.na(MutNucAndEcoCb$FrA.H) & !is.na(MutNucAndEcoCb$Climate_zone) & !is.na(MutNucAndEcoCb$A_G.H) & MutNucAndEcoCb$Climate_zone %in% c("boreal", "polar", "temperate"),]
ColdFishes <- ColdSubset %>% dplyr::select(c("T_G.H", "T_C.H", "T_A.H", "G_T.H", "G_C.H", "G_A.H", "C_T.H" , "C_G.H", "C_A.H", "A_T.H", "A_G.H", "A_C.H"))
ColdFishes <- ColdFishes %>% summarise_all(mean)
...
if (file.exists(here(data_dir, "SimulationData2.csv"))) {
  SimulationData <- read.csv(here(data_dir, "SimulationData2.csv"))
} else {
  cat("Starting simulation...\n") # Initial message
...
#write.csv(SimulationData, here(data_dir, "SimulationData2.csv"))
```
...
WarmFishesNuc <- WarmSubset %>% dplyr::select(FrA.H, FrT.H, FrG.H, FrC.H)
WarmFishesNuc <- WarmFishesNuc %>% summarise_all(mean)


ColdFishesNuc <- ColdSubset %>% dplyr::select(FrA.H, FrT.H, FrG.H, FrC.H)
ColdFishesNuc <- ColdFishesNuc %>% summarise_all(mean)
...
sfn <- ggarrange(plot_cold_water, plot_warm_water, ncol = 2)
sfn
ggsave(here(plots_dir,"Simulation_expected_observed_subset.pdf"), plot = sfn, width = 20, height = 6, dpi = 300)
```

```{r expected_value_subset}
exp_warm <- dplyr::filter(SimulationData, X == 12000)
exp_warm <- exp_warm %>% dplyr::select(FrA.H, FrT.H, FrG.H, FrC.H)
exp_warm <- rbind (exp_warm, c("warm", "warm", "warm", "warm"))
exp_warm <- as.data.frame(t(exp_warm))
...
ggsave(here(plots_dir, "Observed_vs_Expected_Frequencies_subset.png"),
  plot = f2, # Explicitly state which plot to save
  width = 8,          # Width in inches
  height = 8,         # Height in inches
  dpi = 300           # Resolution in dots per inch (good for publications)
