...
sortedCytB <- MutSpecForAnalysisCb %>% arrange(desc(A_G.H))
#write.table(sortedCytB, file = here(output_dir, "Sortet_by_A_G.H.txt"), col.names = TRUE, row.names = FALSE)
...
```{r inertia analysis cytb, message=FALSE, warning=FALSE}
tree = read.tree(file = here(data_dir, "chordates_species.nwk"))
tree <- ape::drop.tip(tree, "Tenebrosternarchus_preto")
MutAndEcoPGLS <- MutAndEco %>% dplyr::select(Species, A_G.H, C_T.H, G_A.H, T_C.H, Temp_avg)
